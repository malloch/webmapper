...
//            print_tree(tree, 0);
        ...
        // copy basic logic from view.js
        // map pane should show potential edge during dragover, with snapping
        // drag over signal should reject self maps, otherwise highlight signal
        // dragleave should unhighlight
        // drop should trigger command.send(map, ...)
        // can be dropped from another webmapper table or signal browser, etc.

        // TODO: use 'signal' as class, centralise drag'n'drop support for all views?
        $('.leaf')
            .bind('dragstart', function(ev) {
                console.log("dragstart", $(ev.target).parents('tr')[0]);
                var dt = ev.originalEvent.dataTransfer;

//                // this works
//                var img = document.createElement("img");
//                img.src = "/images/libmapper_logo_flat_black.png";
//                dt.setDragImage(img, 0, 0);
                ...
                _self.graph.draggingFrom = _self.graph.find_signal(ev.target.id);
//                _self.graph.draggingFromDT = ev.target;

                _self.graph.draggingFrom = {'id': ev.target.id,
                                            'sig': _self.graph.find_signal(ev.target.id),
                                            'table': _self,
                                            'row': $(ev.target).parents('tr')[0]};
                return true;
        });

        $('.leaf')
            .bind('dragenter', function(ev) {
                _self.highlightRow($(ev.currentTarget).parents('tr')[0], true);
                ev.preventDefault();
            });
        $('.leaf')
        ...
        $('.leaf')
            .bind('drop', function(ev) {
                ...
                if (id.startsWith("libmapper://signal ")) {
                    id = id.slice(19).split(" @id ")[0];
                    console.log(">>> map? ", id, " -> ", ev.target.id);
                    mapper.map(id, ev.target.id);
                }
                _self.graph.draggingFrom = null;
                return true;
        });
        $('.leaf')
            .bind('dragend', function(ev) {
                _self.highlightRow(null, true);
                _self.graph.draggingFrom = null;
                return true;
        });
